export interface FastConversationResult { assistantReply: string; intent: 'small_talk' | 'query_menu'; orderActions: []; }
...
export function tryFastConversation(utterance: string): FastConversationResult | null {
  ...
  if (/^(hi|hello|hey|hiya|yo|howdy)( there)?([, ]+(how are you|how are you doing|how's it going))?$/.test(text)) {
  ...
  if (/^(good morning|good afternoon|good evening)$/.test(text)) {
    return { assistantReply: 'Hello! Welcome to VocaBite. What would you like to eat today?', intent: 'small_talk', orderActions: [] };
  }
